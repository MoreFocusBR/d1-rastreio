                    ...
                    try {
                        ...
                        if (!existingRecord) {
                            ...
                            if ((await resCreate.Id) != null) {
                                ...
                                const request = require("superagent");
                                const resListaIntegracao = await request
                                    .post("http://cloud01.alternativa.net.br:2086/root/venda/" +
                                    resCreate.Codigo +
                                    "/ok")
                                    .set("Accept", "application/json")
                                    .set("accept-encoding", "gzip")
...
app.get("/redundanciaCargaVendas", async (req, reply) => {
    ...
    if (ultimaVenda?.Codigo !== undefined) {
        ...
        return reply
            .status(200)
            .send(`codigoInicial ${ultimaVenda?.Codigo - 200}  codigoFinal ${ultimaVenda?.Codigo}`);
    }
});
...
app.get("/cargaVendas", async (request, reply) => {
    ...
    // busca fila de integração Vendas - inicio
    async function pegaVenda(Codigo) {
        try {
            const request = require("superagent");
            const resVenda = await request
    ...
    // busca fila de integração Vendas - fim
    // consome cada item da fila de integração Vendas - inicio
    async function mainConsomeLista(codigoInicial, codigoFinal) {
...
app.get("/testeEmail", async (request, reply) => {
    function substituirMarcador(mensagem, marcador, conteudo) {
    ...
    let emailContentWow = await fs_1.promises.readFile(filePath, "utf-8");
    ...
    if (emailContentDB?.Mensagem) {
        emailContentWow = substituirMarcador(emailContentWow, "conteudoEmail", emailContentDB.Mensagem);
        emailContentWow = substituirMarcador(emailContentWow, "conteudoEmail", emailContentDB.Mensagem);
        emailContentWow = substituirMarcador(emailContentWow, "primeiroNome", "Rodrigo");
        emailContentWow = substituirMarcador(emailContentWow, "numeroPedido", "1234");
        emailContentWow = substituirMarcador(emailContentWow, "dataVenda", "10/09/2024");
        emailContentWow = substituirMarcador(emailContentWow, "enderecoEntrega", `Rua Fernando Machado, 165, Centro, Porto Alegre / RS`);
        emailContentWow = substituirMarcador(emailContentWow, "numeroNotaFiscal", "456789");
        emailContentWow = substituirMarcador(emailContentWow, "transportadoraNome", "BAUER");
        emailContentWow = substituirMarcador(emailContentWow, "previsaoEntrega", "19/09/2024");
        // Opções do e-mail
...
app.get("/updateVendas", async (request, reply) => {
    ...
    async function enviaWhatsStatus(venda) {
        ...
        if (vendaJson.DescricaoStatus == "Nota Fiscal Emitida" &&
            vendaJson.EntregaNome != null) {
            let primeiroNome = vendaJson.EntregaNome.split(" ")[0];
            ...
            if (whatsContentDB?.Mensagem) {
                whatsContent = substituirMarcador(whatsContentDB?.Mensagem, "primeiroNome", primeiroNome);
            }
            ...
            // Conteúdo do e-mail
            // Caminho do arquivo HTML
            const filePath = path.join(__dirname, "", `template-email-resp-NotaFiscalWow.html`);
            // Leitura do conteúdo do arquivo
            let emailContentWow = await fs_1.promises.readFile(filePath, "utf-8");
            const emailContentDB = await prisma.rastreioStatusEmail.findFirst({
                where: {
            ...
            if (emailContentDB?.Mensagem) {
                emailContentWow = substituirMarcador(emailContentWow, "primeiroNome", primeiroNome);
                emailContentWow = substituirMarcador(emailContentWow, "conteudoEmail", emailContentDB.Mensagem);
                emailContentWow = substituirMarcador(emailContentWow, "numeroPedido", `${vendaJson.Codigo}`);
                const dataVendaFormatada = vendaJson.DataVenda.split("-")
                    .reverse()
                    .join("/");
                emailContentWow = substituirMarcador(emailContentWow, "dataVenda", dataVendaFormatada);
                emailContentWow = substituirMarcador(emailContentWow, "enderecoEntrega", `${vendaJson.EntregaLogradouro.split(",")[0]}, ${vendaJson.EntregaLogradouroNumero}, ${vendaJson.EntregaLogradouroComplemento}, ${vendaJson.EntregaBairro}, ${vendaJson.EntregaMunicipioNome} / ${vendaJson.EntregaUnidadeFederativa}`);
                emailContentWow = substituirMarcador(emailContentWow, "numeroNotaFiscal", vendaJson.NotaFiscalNumero);
            }
            // Opções do e-mail
            const mailOptionsRodrigo = {
                from: '"D1Fitness" <naoresponda@d1fitness.com.br>',
                to: `c.albuquerque.rodrigo@gmail.com`, // E-mail do destinatário
                subject: `${vendaJson.EntregaNome.split(" ")[0]}, estamos preparando o envio do seu pedido n. ${vendaJson.Codigo}`,
                text: mensagem,
                html: emailContentWow,
            };
            enviarEmail(mailOptionsRodrigo);
            ...
            // Conteúdo do e-mail
            // Caminho do arquivo HTML
            const filePath = path.join(__dirname, "", `template-email-resp-EnviadoSemPrevisaoWow.html`);
            // Leitura do conteúdo do arquivo
            let emailContentWow = await fs_1.promises.readFile(filePath, "utf-8");
            const emailContentDB = await prisma.rastreioStatusEmail.findFirst({
                where: {
            ...
            const mailOptionsRodrigo = {
                from: '"D1Fitness" <naoresponda@d1fitness.com.br>',
                to: `c.albuquerque.rodrigo@gmail.com`, // E-mail do destinatário
                subject: `${vendaJson.EntregaNome.split(" ")[0]}, seu pedido n. ${vendaJson.Codigo} está a caminho!`,
                text: mensagem,
                html: emailContentWow,
            };
            enviarEmail(mailOptionsRodrigo);
    ...
    while (!todasVendasProcessadas) {
        await prisma.$transaction(async (prismaClient) => {
            ...
            async function processaVenda(prismaClient, venda) {
                try {
                    ...
                    if (resVenda) {
                        ...
                        if (resListaIntegracaoJson.venda &&
                            ...
                            for (const vendaJson of resListaIntegracaoJson.venda) {
                                const { Codigo, ClienteCodigo, ClienteDocumento, TransportadoraCodigo, TransportadoraNome, DataVenda, Entrega, EntregaNome, EntregaEmail, NumeroObjeto, EntregaTelefone, Observacoes, ObservacoesLoja, CodigoStatus, DescricaoStatus, DataHoraStatus, PrevisaoEntrega, CodigoNotaFiscal, DataEntrega, Cancelada, DataEnvio, NotaFiscalNumero, DataColeta, } = vendaJson;
...
app.get("/updateRastreio", async (request, reply) => {
    ...
    while (!todasVendasProcessadas) {
        await prisma.$transaction(async (prismaClient) => {
            ...
            async function enviaWhatsTracking(Telefone, Email, VendaString, momentoTracking, data_hora, previsaoEntrega) {
                ...
                function retornaNomeTransportadora(TransportadoraCodigo) {
                    const transportadora = transportadoras.find((item) => item.TransportadoraCodigo === TransportadoraCodigo);
                ...
                if (momentoTracking == 1 &&
                    ...
                    Venda.Itens) {
                    let whatsContentwow = substituirMarcador(whatsContentDBwow?.Mensagem, "primeiroNome", Venda.EntregaNome.split(" ")[0]);
                    whatsContentwow = substituirMarcador(whatsContentwow, "numeroPedido", `${Venda.Codigo}`);
                    const dataVendaFormatada = Venda.DataVenda.split("-")
                        .reverse()
                        .join("/");
                    whatsContentwow = substituirMarcador(whatsContentwow, "dataVenva", dataVendaFormatada);
                    // {{nomeTransportadoraNotaFiscal}}transportadoraNome
                    // Substui dados da entrega
                    whatsContentwow = substituirMarcador(whatsContentwow, "dadosEntrega", `Dados da entrega\n--------------------\nEndereço de entrega: ${Venda.EntregaLogradouro.split(",")[0]}, ${Venda.EntregaLogradouroNumero}, ${Venda.EntregaLogradouroComplemento}, ${Venda.EntregaBairro}, ${Venda.EntregaMunicipioNome} / ${Venda.EntregaUnidadeFederativa}\nTransportadora: ${retornaNomeTransportadora(Venda.TransportadoraCodigo)}\nNúmero Nota Fiscal: ${Venda.NotaFiscalNumero}`);
                    whatsContentwow = substituirMarcador(whatsContentwow, "previsaoEntrega", previsaoEntrega);
                    // Dispara msg whats
                    ...
                    // Conteúdo do e-mail
                    // Caminho do arquivo HTML
                    const filePath = path.join(__dirname, "", `template-email-resp-${qualMensagem}.html`);
                    // Leitura do conteúdo do arquivo
                    let emailContentWow = await fs_1.promises.readFile(filePath, "utf-8");
                    const emailContentDBWow = await prisma.rastreioStatusEmail.findFirst({
                        where: {
                            Status: qualMensagem,
                        },
                    });
                    if (emailContentDBWow?.Mensagem) {
                        emailContentWow = substituirMarcador(emailContentWow, "primeiroNome", Venda.EntregaNome.split(" ")[0]);
                        emailContentWow = substituirMarcador(emailContentWow, "conteudoEmail", emailContentDBWow.Mensagem);
                        emailContentWow = substituirMarcador(emailContentWow, "numeroPedido", `${Venda.Codigo}`);
                        const dataVendaFormatada = Venda.DataVenda.split("-")
                            .reverse()
                            .join("/");
                        emailContentWow = substituirMarcador(emailContentWow, "dataVenda", dataVendaFormatada);
                        ...
                        // Substui dados da entrega
                        emailContentWow = substituirMarcador(emailContentWow, "enderecoEntrega", `${Venda.EntregaLogradouro.split(",")[0]}, ${Venda.EntregaLogradouroNumero}, ${Venda.EntregaLogradouroComplemento}, ${Venda.EntregaBairro}, ${Venda.EntregaMunicipioNome} / ${Venda.EntregaUnidadeFederativa}`);
                        emailContentWow = substituirMarcador(emailContentWow, "transportadoraNome", `${retornaNomeTransportadora(Venda.TransportadoraCodigo)}`);
                        emailContentWow = substituirMarcador(emailContentWow, "numeroNotaFiscal", `${Venda.NotaFiscalNumero}`);
                        emailContentWow = substituirMarcador(emailContentWow, "previsaoEntrega", previsaoEntrega);
                        // Opções do e-mail
                        const mailOptions = {
                            from: '"D1Fitness" <naoresponda@d1fitness.com.br>',
                            to: `${Email}`, // E-mail do destinatário
                            subject: `${Venda.EntregaNome.split(" ")[0]}, seu pedido n. ${Venda.Codigo} está a caminho!`,
                    ...
                    for (const row of itensVenda) {
                        const { Codigo, ProdutoReferencia, ProdutoBarras, ProdutoBundleCodigo, VendaCodigo, ProdutoCodigo, PrecoUnitarioVenda, PrecoUnitarioCusto, EmbaladoParaPresente, ValorEmbalagemPresente, Quantidade, AtributosEspeciais, ItemNome, ItemDescontoPercentual, ItemDescontoValor, ItemValorBruto, ItemValorLiquido, Servico, Movimentacao, } = row;
                        textoItens += `${Quantidade.split(".")[0]} X ${ItemNome}\n`;
                    }
                    // Substitui dados itens
                    whatsContentwow = substituirMarcador(whatsContentwow, "dadosPedido", `\n\nItens do Pedido n. ${Venda.Codigo}\n--------------------\n${textoItens}`);
                    // Dispara msg whats
                    const bodyWhats1 = `{"phone": "55${Telefone}","message": "${whatsContentwow}"}`;
                    const resZAPI = await request
                        .post("https://api.z-api.io/instances/39BD5CDB5E0400B490BE0E63F29971E4/token/996973B6263DE0E95A59EF47/send-text")
  ...
  `;
                        emailContentWow = substituirMarcador(emailContentWow, "listaItens", `${htmlTabelaItens}`);
                        // Opções do e-mail
                        const mailOptions = {
                            from: '"D1Fitness" <naoresponda@d1fitness.com.br>',
                            to: `${Email}`, // E-mail do destinatário
                            subject: `${Venda.EntregaNome.split(" ")[0]}, seu pedido n. ${Venda.Codigo} está chegando!`,
                            text: whatsContentwow,
                            html: emailContentWow,
                        };
                        enviarEmail(mailOptions);
                    ...
                    if (emailContentDBWow?.Mensagem) {
                        ...
                        const mailOptions = {
                            from: '"D1Fitness" <naoresponda@d1fitness.com.br>',
                            to: `${Email}`, // E-mail do destinatário
                            subject: `${Venda.EntregaNome.split(" ")[0]}, seu pedido n. ${Venda.Codigo} foi entregue!`,
                            text: whatsContentwow,
                            html: emailContentWow,
                        };
                        enviarEmail(mailOptions);
            ...
            async function processaVendaSSW(prismaClient, vendaString) {
                const venda = JSON.parse(vendaString);
                try {
                    const resVenda = await pegaVenda(venda.Codigo);
                    if (resVenda) {
                        ...
                        if (resListaIntegracaoJson.venda &&
                            resListaIntegracaoJson.venda.length > 0) {
                            for (const vendaJson of resListaIntegracaoJson.venda) {
                                const { Codigo, ClienteCodigo, ClienteDocumento, TransportadoraCodigo, TransportadoraNome, DataVenda, Entrega, EntregaNome, EntregaEmail, NumeroObjeto, EntregaTelefone, Observacoes, ObservacoesLoja, CodigoStatus, DescricaoStatus, DataHoraStatus, PrevisaoEntrega, CodigoNotaFiscal, DataEntrega, Cancelada, DataEnvio, NotaFiscalNumero, DataColeta, ItensVenda, } = vendaJson;
                                // Pega ocorrencias rastreio
                                // SSW
                                const request = require("superagent");
                                // Busca dados NFe
                                const resNfe = await request
                                    .get(`http://cloud01.alternativa.net.br:2086/root/nfe/${venda.CodigoNotaFiscal}`)
                                    .set("Accept", "application/json")
                                    .set("accept-encoding", "gzip")
                                    .set("X-Token", "7Ugl10M0tNc4M8KxOk4q3K4f55mVBB2Rlw1OhI3WXYS0vRs");
                                ...
                                if (SSWocorrenciasJson.success) {
                                    for (const row of SSWocorrenciasJson.documento.tracking) {
                                        const { data_hora, dominio, filial, cidade, ocorrencia, descricao, tipo, data_hora_efetiva, } = row;
                                        // Verifica se é ocorrencia Previsão, LastMile ou Entregue
                                        if (/Previsao de entrega: /i.test(descricao)) {
                                            const previsaoEntregaArray = descricao.split(`Previsao de entrega: `);
                                            const previsaoEntregaComPonto = previsaoEntregaArray[1].split(".");
                                            const previsaoEntrega = previsaoEntregaComPonto[0];
                                            console.log("Inserindo PrevisaoEntregaRastreio pelo SSW");
                                            await prismaClient.venda.update({
                                                where: { Codigo: venda.Codigo },
                                                data: {
                                            ...
                                            if (!jaAvisouPrevisaoEntrega) {
                                                ...
                                                await enviaWhatsTracking(telefoneRodrigo, emailRodrigo, JSON.stringify(vendaJson), 1, data_hora, previsaoEntrega);
                                                // Deixando apena whats pro Renan
                                                await enviaWhatsTracking(telefoneRenan, emailRodrigo, JSON.stringify(vendaJson), 1, data_hora, previsaoEntrega);
                                                await enviaWhatsTracking(vendaJson.EntregaTelefone, vendaJson.EntregaEmail, JSON.stringify(vendaJson), 1, data_hora, previsaoEntrega);
                                            ...
                                            if (!jaAvisouLastMile) {
                                                ...
                                                const telefoneRenan = "48988038546";
                                                const emailRenan = "renan@d1fitness.com.br";
                                                await enviaWhatsTracking(telefoneRodrigo, emailRodrigo, JSON.stringify(vendaJson), 2, data_hora, "");
                                                // Deixando apena o whats pro Renan
                                                await enviaWhatsTracking(telefoneRenan, emailRodrigo, JSON.stringify(vendaJson), 2, data_hora, "");
                                                await enviaWhatsTracking(vendaJson.EntregaTelefone, vendaJson.EntregaEmail, JSON.stringify(vendaJson), 2, data_hora, "");
                                            ...
                                            if (!jaAvisouEntregue) {
                                                ...
                                                const telefoneRenan = "48988038546";
                                                const emailRenan = "renan@d1fitness.com.br";
                                                await enviaWhatsTracking(telefoneRodrigo, emailRodrigo, JSON.stringify(vendaJson), 3, data_hora, "");
                                                // Deixando apena whats pro Renan
                                                await enviaWhatsTracking(telefoneRenan, emailRodrigo, JSON.stringify(vendaJson), 3, data_hora, "");
                                                await enviaWhatsTracking(vendaJson.EntregaTelefone, vendaJson.EntregaEmail, JSON.stringify(vendaJson), 3, data_hora, "");
...
app.get("/updateRastreioAvaliacao", async (request, reply) => {
    ...
    while (!todasVendasProcessadas) {
        await prisma.$transaction(async (prismaClient) => {
            ...
            async function enviaWhatsAvaliacao(VendaString) {
                const Venda = JSON.parse(VendaString);
                const request = require("superagent");
                let qualMensagem = "";
                // Mensagem Wow
...
app.get("/cargaNfes", async (request, reply) => {
    ...
    async function mainInsereNfes(codigoInicial, codigoFinal) {
        const request = require("superagent");
        const resListaVendas = await request
            .get(`${API_URL}/vendas?codigoInicial=${codigoInicial}&codigoFinal=${codigoFinal}`)
            .set("Accept", "application/json");
        if (resListaVendas) {
            const resListaVendasJson = await JSON.parse(resListaVendas.text);
            resListaVendasJson.forEach(async (venda) => {
                try {
                    const request = require("superagent");
                    const resNfe = await request
                        .get(`http://cloud01.alternativa.net.br:2086/root/nfe/${venda.CodigoNotaFiscal}`)
                        ...
                        .set("X-Token", "7Ugl10M0tNc4M8KxOk4q3K4f55mVBB2Rlw1OhI3WXYS0vRs");
                    const NfeJson = JSON.parse(resNfe.text);
                    const { Codigo, CodigoVenda, CodigoCliente, DataEmissao, HoraEmissao, HoraSaida, Nfe, Nfce, NotaFiscalNumero, TransportadoraCodigo, TransportadoraNome, MeioTransporte, NumeroObjeto, NotaFiscalEletronica, Cancelada, MotivoCancelamento, } = NfeJson.nfe[0];
                    ...
                    if (!existingRecord && CodigoVenda != null) {
                        ...
                        // Realiza o UPDATE da venda já cadastrada
                        console.log("Realizando Update da NFe da Venda: " +
                            CodigoVenda +
                            "NÂO IMPLEMENTADO");
                        /*
...
app.get("/retornaStatusEntrega", async (request, reply) => {
    ...
    return reply
        .status(200)
        .send(JSON.parse(JSON.stringify(`{ "ocorrencias": "${resStatusEntregaBlip.text}" }`)));
});
...
app.get("/retornaStatusEntregaBlip", async (request, reply) => {
    ...
    // consome cada item da Lista Vendas - inicio
    async function mainBuscaOcorrencias(cpfcnpj) {
        const request = require("superagent");
        const resUltimaVendaCpfCnpj = await request
            .get(`${API_URL}/ultimaVendaCPFCNPJ?cpfcnpj=${cpfcnpj}`)
            .set("Accept", "application/json");
        if (await resUltimaVendaCpfCnpj) {
            const resUltimaVendaCpfCnpjJson = await JSON.parse(resUltimaVendaCpfCnpj.text);
            ...
            try {
                if ((await resUltimaVendaCpfCnpjJson.venda[0].NotaFiscalNumero) > 0) {
                    ...
                    const resNfe = await requestNF
                        .get(`http://cloud01.alternativa.net.br:2086/root/nfe/${resUltimaVendaCpfCnpjJson.venda[0].CodigoNotaFiscal}`)
                        .set("Accept", "application/json")
                        .set("accept-encoding", "gzip")
                        .set("X-Token", "7Ugl10M0tNc4M8KxOk4q3K4f55mVBB2Rlw1OhI3WXYS0vRs");
                    const NfeJson = JSON.parse(resNfe.text);
                    ...
                    if (NotaFiscalEletronica > 0) {
                        console.log(`Transportadora ${TransportadoraNome}. Buscando ocorrências da Nfe: ` +
                            NotaFiscalEletronica);
                        // Busca Ocorrências Bauer, Aceville, Gobor, TPL
                        if (TransportadoraNome == "BAUER" ||
                            ...
                            const SSWocorrenciasJson = JSON.parse(resSSW.text);
                            if (SSWocorrenciasJson.success) {
                                SSWocorrenciasJson.documento.tracking.forEach((row, index) => {
                                    const { data_hora, dominio, filial, cidade, ocorrencia, descricao, tipo, data_hora_efetiva, } = row;
                        ...
                        // Busca Ocorrências MODULAR
                            ...
                            const resMODULARocorrencias = await requestMODULARocorrencias
                                ...
                                .set("Content-Type", "multipart/form-data")
                                .field("dados[filialorigem]", `${MODULARJson.notas[0].Filial_Origem}`)
                                .field("dados[controle]", `${MODULARJson.notas[0].Controle}`)
                                .set("Accept", "application/json");
                            const MODULARocorrenciasJson = JSON.parse(resMODULARocorrencias.text);
                            ...
                            const dadosFormatados = formatarDados(MODULARocorrenciasJson.posicao);
                        ...
                        // Busca Ocorrências MANN
                            ...
                            const payloadMANN000000 = {
                                user: {
                                    ...
                                    filialNum: 27,
                                    auth: {
                                        value: "SSM13wnMAzrqbri/I3z0B6cw82xSDT+iBQSvDOxK8IrEv58kZnLDoO13DWnmPZEP8q7QO6AqQ0XEDW+7noeS60yyjQQlvKGU",
                            ...
                            const MANNocorrenciasJson001 = MANNocorrenciasJson00.report;
                            if (MANNocorrenciasJson001 != null) {
                                ...
                                const report = MANNocorrenciasJson.report;
                        ...
                        // Busca Ocorrências MOVVI
                            ...
                            const resMOVVI = await request
                                .get(`https://apimovvi.meridionalcargas.com.br/api/rastrear-carga/${cpfcnpj2}/${NotaFiscalNumero}`)
                                .set("Accept", "application/json");
                            const MOVVIocorrenciasJson = JSON.parse(resMOVVI.text);
                            if (MOVVIocorrenciasJson && MOVVIocorrenciasJson.ocorrencias) {
                                MOVVIocorrenciasJson.ocorrencias.forEach((row, index) => {
                        ...
                        // Busca Ocorrências JAMEF
                            ...
                            if (JAMEFocorrenciasJson &&
                                JAMEFocorrenciasJson.conhecimentos[0].historico) {
                                JAMEFocorrenciasJson.conhecimentos[0].historico.forEach((row, index) => {
    ...
    async function formatarOcorrencias() {
        const retornoEndpoint = await JSON.parse("" + retornoEndpointString); // Suponha que obterRetornoEndpoint() é a função que retorna a promessa
        let resultadoFormatado = "";
    ...
    if (TransportadoraVenda == "MODULAR") {
        ...
        if ((await retornoEndpointFormatado) != "Aguardando emissão da Nota Fiscal.") {
...
app.get("/rastreioChat/:idMensagem", async (request, reply) => {
    const idMensagem = request.parametro;
...
app.post("/zapi", async (request, reply) => {
    ...
    if (typeof request.body === "object" && request.body !== null) {
        const telefoneCliente = request.body.phone;
        const mensagemCliente = request.body.text
            .message;
        let mensagem = "";
        let Etapa = 0;
        ...
        const formattedDate = `${now.getFullYear()}-${(now.getMonth() + 1)
            ...
            .toString()
            .padStart(2, "0")}`;
        function valida_cpfcnpj(inputCPFCNPJ) {
            ...
            function CPFouCNPJ(inputCPFCNPJ) {
                const contador = inputCPFCNPJ.replace(/[^0-9]/g, "").length;
                if (contador == 0) {
            ...
            function validaCNPJ(CNPJ) {
                ...
                if (parseInt(CNPJ.charAt(12)) != a[12] ||
        ...
        return reply.status(500).send(console_1.error);
    }
});
// Webhook Z-API -> Blip Rastreio - inicio
...
app.post("/whatsrastreio", async (request, reply) => {
    ...
    if (typeof bodyBlip === "object" && bodyBlip !== null) {
        ...
        const telefoneCliente = bodyBlip.phone;
        const blocoCliente = bodyBlip.bloco;
        let mensagem = "";
        if (blocoCliente == "solicita_cfpcnpj") {
            ...
            const ocorrencias = bodyBlip.ocorrencias;
...
app.post('/enviaMsgAvaliacao', zapi_1.enviaMsgAvaliacao);
// Chatbot pós-venda mensagem 25 dias - fim
// Chatbot pós-venda - início
app.post('/zapi2', zapi_1.handleIncomingMessage);
